...
## DDA-directlfq-Protein-DEP2 workflow.

```{r pg_directlfq2,results='hide',message=FALSE,warning=FALSE}
setwd(example_path)
...
## Aggregation
pe_direct2 <- aggregate_pe(pe, aggrefun = "directLFQ", fcol = "Proteins", 
                           reserve = c("Gene.names","Reverse","Potential.contaminant"), i = "peptideRaw")
se_pepdire2 <- pe2se(pe_direct2, names = "Gene.names", ids = "smallestProteingroups")

## Filter & normalize & Impute
filt_pepdire2 <- filter_se(se_pepdire2,thr = 0, fraction = 0.5,filter_formula = ~.n > 1)
filt_pepdire3 <- filter_se(filt_pepdire2, filter_formula = ~ Reverse != '+' & Potential.contaminant !="+" )
norm_pepdire2 <- normalize_vsn(filt_pepdire3)
set.seed(10000)
imp_pepdire2 <- DEP2::impute(norm_pepdire2,fun = "RF")

## Differential test
diff_pepdire2 <- test_diff(imp_pepdire2,type = "control", control = "A1", fdr.type = "BH")
dep_pepdire2 <- add_rejections(diff_pepdire2, alpha = 0.05, lfc = 1)
...
## DDA-peptide-directlfq-DEP2 workflow2.

This pipeline bypass the MaxLFQ summarization and instead performs re-aggregation of protein intensity based on peptide quantities.


```{r pg_directlfq1,results='hide',message=FALSE,warning=FALSE}
setwd(example_path)
mq_pepfile <- "./A_spikeIn/MaxQuant_output/peptides.txt.gz"
mq_pep <- read.csv(mq_pepfile,sep = "\t")
getwd()
...
## Aggregation
pe_direct <- aggregate_pe(pe_direct, aggrefun = "directLFQ", fcol = "Proteins", reserve = "Gene.names", i = "peptideImp")
se_pepdire1 <- pe2se(pe_direct, names = "Gene.names", ids = "smallestProteingroups")
filt_pepdire1<- filter_se(se_pepdire1,filter_formula = ~.n > 1) 
norm_pepdire1 <- normalize_vsn(filt_pepdire1)


## Differential test
diff_pepdire1 <- test_diff(norm_pepdire1,type = "control", control = "A1", fdr.type = "BH")
dep_pepdire1 <- add_rejections(diff_pepdire1, alpha = 0.01)
```
...
direct_df1 <- get_df_wide(dep_pepdire1) %>% as.data.frame %>%  
  mutate( meanQuant = rowMeans(dplyr::select(., matches("^(A1|B1)_(1|2|3)$"))) ) %>% 
  dplyr::select(name, ID, 
                L2FC = B1_vs_A1_diff, pval = B1_vs_A1_p.val, qval = B1_vs_A1_p.adj, t.statistic = B1_vs_A1_t.stastic, meanQuant) %>% 
  left_join(protein_info2) %>% 
  cbind(assay(dep_pepdire1))

direct_df2 <- get_df_wide(dep_pepdire2) %>% as.data.frame %>%  
  mutate( meanQuant = rowMeans(dplyr::select(., matches("^(A1|B1)_(1|2|3)$"))) ) %>% 
  dplyr::select(name, ID, 
                L2FC = B1_vs_A1_diff, pval = B1_vs_A1_p.val, qval = B1_vs_A1_p.adj, t.statistic = B1_vs_A1_t.stastic, meanQuant) %>% 
  left_join(protein_info2) %>% 
  cbind(assay(dep_pepdire2))
...
df_list <- list("DDA-Protein-DEP2" = PG_df, 
                ...
                "DDA-Peptide-dlfq2-DEP2" = direct_df1,
                "DDA-Peptide-dlfq-DEP2" = direct_df2,
                "DIA-DEP2" = DIA_df)
...
```{r}
# p_lfc
pdf("p_lfc.pdf",height = 10,width = 14)
p_lfc
dev.off()
```
...
# p_lfc
pdf("p_lfcerr.pdf",height = 10,width = 14)
p_lfcerr
dev.off()
```
...
pdf("p_lfctrend.pdf",height = 10,width = 14)
p_lfctrend
dev.off()
```
...
# p_cvs
pdf("p_cvs.pdf",height = 10,width = 14)
p_cvs
dev.off()
```
...
ggplot(roc_df) +
  ...
  theme_bw() +
  theme(panel.grid = element_blank()) -> p_roc

pdf("p_roc.pdf",height = 10,width = 14)
p_roc
dev.off()
```
...
ggplot(roc_df) +
  ...
  coord_equal() +
  theme_bw() +
  theme(panel.grid = element_blank()) -> p_fdr

pdf("p_fdr.pdf",height = 10,width = 14)
p_fdr
dev.off()
```
...
pdf("p_sig.pdf",height = 10,width = 14)
p_sig / p_sig_fraction
dev.off()
```
