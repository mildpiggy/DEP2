...
filter_peptide = function(peptidedata){
  data = peptidedata
  data %<>% filter(Reverse=="") %>% filter(Potential.contaminant=="")
  data$PTM_ID = paste(data$Protein,"_", data$Amino.acid , data$Position ,sep="")
  data$PTM_name = paste(ifelse(data$Gene.names=="" , data$Protein , data$Gene.names), "_" , data$Amino.acid , data$Position,sep="")
...
PTM_get_results = function (dep)
{
  ...
  pval <- as.data.frame(row_data) %>% column_to_rownames("name") %>%
    select(ends_with("p.val"), ends_with("p.adj"), ends_with("significant")) %>%
    rownames_to_column()
...
PTM_get_table = function (results, type = c("centered", "contrast"))
{
  ...
  significant <- results %>% filter(significant) %>% select(-significant)
  if (type == "centered") {
                          ...
                          plot = TRUE, if_mysplit = FALSE, mysplit = NULL, if_rowname_color = FALSE, ...)
{
  ...
  ht1 = ComplexHeatmap::Heatmap(df,
                                ...
                                name = legend,
                                row_names_gp = grid::gpar(fontsize = row_font_size, col = row_name_color),
                       ...
                       indicate = NULL, font_size = 12, plot = TRUE, add_values = FALSE, value_size = 10, digits = 2, ...)
{
  ...
  ht1 = Heatmap(dist_mat, col = circlize::colorRamp2(seq(0,
                ...
                name = "Gower's distance", column_names_gp = grid::gpar(fontsize = font_size),
                row_names_gp = gpar(fontsize = font_size), top_annotation = ha1, cell_fun = if(add_values) {function(j, i, x, y, width, height, fill) {
                  grid.text(sprintf(paste("%.", digits, "f", sep = ""), dist_mat[i, j]), x, y, gp = grid::gpar(fontsize = value_size))}} else {NULL},
                ...)
  if (plot) {
    draw(ht1, heatmap_legend_side = "top")
  }
                      ...
                      ...)
{
  ...
  ht1 = Heatmap(cor_mat, col = circlize::colorRamp2(seq(lower,
                ...
                row_names_gp = gpar(fontsize = font_size), top_annotation = ha1, cell_fun = if(add_values) {function(j, i, x, y, width, height, fill) {
                  grid.text(sprintf(paste("%.", digits, "f", sep = ""), cor_mat[i, j]), x, y, gp = grid::gpar(fontsize = value_size))}} else {NULL},
                ...)
  if (plot) {
    draw(ht1, heatmap_legend_side = "top")
  }
...
plot_missval <- function (se, ...)
{
  ...
  ht2 = ComplexHeatmap::Heatmap(missval, col = c("white", "black"),
                                ...
                                column_names_gp = grid::gpar(fontsize = 16), heatmap_legend_param = list(at = c(0,
...
my_plot_volcano <- function(dep, contrast, adjusted = FALSE ,labelWay = "all significant",showNum = 20, chooseTolabel = NULL,fontSize = 9, dotsize = 2, same_width = TRUE, plot = TRUE, fcCutoff, linetype = 4, adjpCutoff = 0.05, if_peptide_color = FALSE, label.rectangle = FALSE, stroke = 0.6) {
  library(ggpubr)
  library(ggthemes)
  #  if (is.integer(label_size))
  #     label_size <- as.numeric(label_size)
  ...
  signif <- match(paste(contrast, "_significant", sep = ""),
                  colnames(row_data))
  peptide_probabilities = grep(".Probabilities",colnames(row_data))
  peptides = row_data[,peptide_probabilities] %>% sapply(.,function(x){sum((row_data[,peptide_probabilities]==x))})
  df <- data.frame(x = row_data[, diff], y = -log10(row_data[,p_values]),
                   significant = row_data[, signif], name = row_data$name, Peptides = peptides) %>%
    dplyr::filter(!is.na(significant)) %>% arrange(significant)
  name1 <- gsub("_vs_.*", "", contrast)
  name2 <- gsub(".*_vs_", "", contrast)
  xlimit <- ceiling(max(c(abs(min(df$x)), abs(max(df$x)))))
  ...
  if(if_peptide_color){
    ...
    test[!test$change == "stable",]$change[which(test[!test$change == "stable",]$Peptides >= 3)] = "peptide_3"  ##peptide 进行了修改
  }
  ...
  volcanoPlot <-  ggscatter(test,
                            ...
                            repel = T,
                            palette = c("down" = "#0099CC", 'stable' = "grey", "up" = "orange3", "peptide_2" = "purple", "peptide_3" = "blue") ,# #CC9933,#FC4E07,lightcoral,#0099CC,#00AFBB
                            label.select = labels,
    ...
    geom_vline(xintercept=c(-fcCutoff,fcCutoff),linetype=linetype, col="grey",lwd=0.6) +
    ...
    geom_point(size = dotsize, shape = 21, color = "black", stroke = stroke, position = "identity", data = for_label)
...
# res: giveGO_res_and_table funtion return value
# ShowCategory: number of go terms of plot
# color: means categorySize, p.adjust" or "pvalue"
# circular: if circular layout
# colorEdge: if colorEdge
# df_with_lg2fc: if df_with_lg2fc , the same as function goAnalysis
my_cnetplot <- function(res, ShowCategory = 5, color = "p.adjust", circular = TRUE, colorEdge = TRUE, df_with_lg2fc = FALSE, ont = "BP") {
  if(ont == "ALL") {
    sig_res_for_ALL <- res$sig_res %>% clusterProfiler.dplyr::arrange(p.adjust)
    if(df_with_lg2fc) {
      try(cnetplot(sig_res_for_ALL, showCategory = ShowCategory, categorySize=color, foldChange = res$de, circular = circular, colorEdge = colorEdge), silent = T)
    } else {
      try(cnetplot(sig_res_for_ALL, showCategory = ShowCategory, categorySize=color, foldChange=NULL, circular = circular, colorEdge = colorEdge), silent = T)
    }

  } else {
    if(df_with_lg2fc) {
      try(cnetplot(res$sig_res, showCategory = ShowCategory, categorySize=color, foldChange = res$de, circular = circular, colorEdge = colorEdge), silent = T)
    } else {
      try(cnetplot(res$sig_res, showCategory = ShowCategory, categorySize=color, foldChange=NULL, circular = circular, colorEdge = colorEdge), silent = T)
    }
  }
}
...
# colorEdge: if colorEdge
# df_with_lg2fc: if df_with_lg2fc , the same as function goAnalysis
cnetplot_for_react_kegg <- function(res, ShowCategory = 5, color = "p.adjust", circular = TRUE, colorEdge = TRUE, df_with_lg2fc = FALSE) {
  if(df_with_lg2fc) {
    try(cnetplot(res$sig_res, showCategory = ShowCategory, categorySize=color, foldChange = res$de, circular = circular, colorEdge = colorEdge), silent = T)
  } else {
    try(cnetplot(res$sig_res, showCategory = ShowCategory, categorySize=color, foldChange=NULL, circular = circular, colorEdge = colorEdge), silent = T)
                          ...
                          indicate = NULL, font_size = 12, plot = TRUE, add_values = FALSE, value_size = 10, digits = 2, ...)
{
  ...
  ht1 = Heatmap(dist_mat, col = circlize::colorRamp2(seq(0, max, ((max)/7)), if (pal_rev) {rev(RColorBrewer::brewer.pal(8, pal))}
                ...
                row_names_gp = grid::gpar(fontsize = font_size), top_annotation = ha1, cell_fun = if(add_values) {function(j, i, x, y, width, height, fill) {
...
#  * @return {[type]}                        [description]
#  */
plot_heatmap_rnaseq <- function (data, row_data, type = c("raw", "centered"), manual = FALSE, manual_name = NULL, kmeans = FALSE, k = 6, col_limit = 6, clustering_distance = c("euclidean", "maximum", "manhattan", "canberra", "binary", "minkowski", "pearson", "spearman", "kendall", "gower"), row_font_size = 6, col_font_size = 10,plot = TRUE, if_mysplit = FALSE, mysplit = NULL, ...)
{
  library(ComplexHeatmap)
  library(tidyr)
  library(tibble)
...
#  * @return {[type]}                 [description]
#  */
plot_volcano_rnaseq <- function(res, contrast, adjusted = FALSE ,labelWay = "up",showNum = 20, chooseTolabel = NULL,fontSize = 9, dotsize = 2, same_width = TRUE, plot = TRUE, fcCutoff = log2(2), linetype = 4, adjpCutoff = 0.05, label.rectangle = FALSE, stroke = 0.6) {
  library(ggpubr)
  library(ggthemes)
  library(DEP)
  ...
  volcanoPlot <-  ggscatter(test,
                            ...
                            font.label = c(fontSize, "plain") ,
                            repel = T,
                            palette = c("down" = "#0099CC", 'stable' = "grey", "up" = "orange3") ,# #CC9933,#FC4E07,lightcoral,#0099CC,#00AFBB
                            label.select = labels,
                            label.rectangle = label.rectangle) +
    theme_base() +
    #adjust pvalue volcano in DEP is very ugly, Basically not used, so no need to add the limit line based on padj
...
#  * @return {[type]}                   [description]
#  */
plot_pro_rna_heatmap <- function(rna, pro, gene, row_font_size = 6, col_font_size = 8, column_title_size = 10, Numcol_pro = 2, Numcol_rna = 2, cluster_columns = F) {
  library(dplyr)
  library(circlize)
  library(RColorBrewer)
  library(ComplexHeatmap)
  ...
  df <- merge(df_rna, pro, by.x = "name", by.y = "name", all.x = TRUE)
  ...
  dat = df[ , c(match("gene", colnames(df)), match("name_rna", colnames(df)), match("row_color_RNA", colnames(df)), 2:(match("name_rna", colnames(df)) -1), match("name_pro", colnames(df)), match("row_color_pro", colnames(df)), (match("gene", colnames(df)) +1):(match("name_pro", colnames(df)) -1))]
  names(dat) = gsub("\\.1","",colnames(dat))

  RNA = dat[ , (match("row_color_RNA", colnames(dat)) + 1) : (match("name_pro", colnames(dat)) - 1)]
  PRO = dat[ , (match("row_color_pro", colnames(dat)) + 1) : ncol(dat)]
  rownames(RNA) = dat$gene
  rownames(PRO) = dat$gene

  ################### PRO heatmap
  row_name_color_pro = df$row_color_pro

  dat_pro = PRO - rowMeans(PRO)
  # set NA to 0
  dat_pro[is.na(dat_pro)] = 0


  ht1=Heatmap(dat_pro, col = colorRamp2(seq(-Numcol_pro, Numcol_pro, (Numcol_pro/5)), rev(brewer.pal(11, "RdBu"))),
              na_col = "grey",row_names_gp = gpar(fontsize = row_font_size, col = row_name_color_pro),
              column_names_gp = gpar(fontsize = col_font_size),
  ...
  ################### RNA heatmap
  row_name_color_rna = df$row_color_RNA

  dat_rna = RNA - rowMeans(RNA)
  #set NA to 0
  dat_rna[is.na(dat_rna)] = 0

  ht2=Heatmap(dat_rna, col = colorRamp2(seq(-Numcol_rna, Numcol_rna, (Numcol_rna/5)), rev(brewer.pal(11, "RdBu"))),
              na_col = "grey",row_names_gp = gpar(fontsize = row_font_size, col = row_name_color_rna),
              column_names_gp = gpar(fontsize = col_font_size),
              row_names_side = "left", column_names_side = "top", show_row_names = T, show_column_names = T,
              heatmap_legend_param = list(color_bar = "continuous",title_position = "topleft"), name = "RNA", cluster_columns = cluster_columns, row_km = 1,cluster_rows = F, row_order = rw, column_title = "RNA centered normalized count", column_title_gp = gpar(fontsize = column_title_size))#width = unit(6, "cm")
  ...
  return(list(ht_pro = ht_pro, ht_rna = ht_rna, ht_merge = hhh, dat = dat))
}
