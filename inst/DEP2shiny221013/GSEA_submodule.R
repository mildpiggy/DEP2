  ...
  tabPanel(tab_name,
           ...
           fluidRow(
             ...
             column(width = 5,
                    tabBox(title = "Result Plots", width = 12,
                           tabPanel(title = "Bar plot",
                                    fluidRow(box(checkboxInput(ns("gseOther_Select_bar"), label = "Selected genesets"), width = 6)),
                                    fluidRow(
                                    ...
                                    fluidRow(
                                      plotOutput(ns("gseOther_barplot"), height = 500),
                                      downloadButton(ns('download_gseOther_barplot'), 'Save')
                                    ),
                                    shinyBS::bsTooltip(ns("gseOther_Select_bar"), "Display the selected genesets in res table or top genesets(ordered by p)", "top", options = list(container = "body")),
                           ...
                           tabPanel(title = "Dot plot",
                                    fluidRow(box(checkboxInput(ns("gseOther_Select_dot"), label = "Selected genesets"), width = 6)),
                                    fluidRow(
                                    ...
                                    shinyBS::bsTooltip(ns("gseOther_Select_dot"), "Display the selected genesets in res table or top genesets(ordered by p)", "top", options = list(container = "body")),
                           ...
                           tabPanel(title = "Heatplot",
                                    fluidRow(box(checkboxInput(ns("gseOther_Select_heat"), label = "Selected genesets"), width = 6)),
                                    fluidRow(
                                    ...
                                    shinyBS::bsTooltip(ns("gseOther_Select_heat"), "Display the selected genesets in res table or top genesets(ordered by p)", "top", options = list(container = "body")),
                           ...
                           tabPanel(title = "Cnetplot",
                                    fluidRow(box(checkboxInput(ns("gseOther_Select_cnet"), label = "Selected genesets"), width = 6)),
                                    fluidRow(
                                    ...
                                    shinyBS::bsTooltip(ns("gseOther_Select_cnet"), "Display the selected genesets in res table or top genesets(ordered by p)", "top", options = list(container = "body")),
                           ...
                           tabPanel(title = "Emaplot",
                                    fluidRow(box(checkboxInput(ns("gseOther_Select_ema"), label = "Selected genesets"), width = 6)),
                                    fluidRow(
                                    ...
                                    shinyBS::bsTooltip(ns("gseOther_Select_ema"), "Display the selected genesets in res table or top genesets(ordered by p)", "top", options = list(container = "body")),
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      gseOther_barplot_input <- reactive({
        ...
        progress <- shiny::Progress$new()
        progress$set(message = "Plotting", value = 0.66)
        if(input$gseOther_Select_bar){
          if(is.null(input$gseOther_Table_rows_selected)) return(NULL)
          res <- res_gseOther()$sig_res %>% filter(.,1:nrow(.) %in% input$gseOther_Table_rows_selected)
        }else{
          res <- res_gseOther()$sig_res
        }
        on.exit(progress$close())
        enrichplot:::barplot.enrichResult(res, showCategory = input$gseOther_ShowCategory_bar, color = input$gseOther_color, x = "NES", split="phenotype") + labs(y = "NES")
      })

      gseOther_dotplot_input <- reactive({
        progress <- shiny::Progress$new()
        progress$set(message = "Plotting", value = 0.66)
        if(input$gseOther_Select_dot){
          if(is.null(input$gseOther_Table_rows_selected)) return(NULL)
          res <- res_gseOther()$sig_res %>% filter(.,1:nrow(.) %in% input$gseOther_Table_rows_selected)
        }else{
          res <- res_gseOther()$sig_res
        ...
        on.exit(progress$close())
        enrichplot:::dotplot(res, showCategory = input$gseOther_ShowCategory_dot, color = input$gseOther_color, x = "NES", split="phenotype")
      })

      gseOther_heatplot_input <- reactive({
        withProgress(message = 'Plotting', value = 0.66, {
          if(input$gseOther_Select_heat){
            if(is.null(input$gseOther_Table_rows_selected)) return(NULL)
            res <- res_gseOther()$sig_res %>% filter(.,1:nrow(.) %in% input$gseOther_Table_rows_selected)
          }else{
            res <- res_gseOther()$sig_res
          }
          try(Heatplot(res, showCategory = input$gseOther_ShowCategory_heat, foldChange = res_gseOther()$de), silent = T)
        })
      })

      gseOther_cnetplot_input <- reactive({
        withProgress(message = 'Plotting', value = 0.66, {
          if(input$gseOther_Select_cnet){
            if(is.null(input$gseOther_Table_rows_selected)) return(NULL)
            res <- res_gseOther()$sig_res %>% filter(.,1:nrow(.) %in% input$gseOther_Table_rows_selected)
          }else{
            res <- res_gseOther()$sig_res
          }
          try(Cnetplot(x = res, showCategory = input$gseOther_ShowCategory_cnet, foldChange = res_gseOther()$de, circular = input$gseOther_circular_cnet, colorEdge = TRUE), silent = T)
        })
      })

      gseOther_emaplot_input <- reactive({
        withProgress(message = 'Plotting', value = 0.66, {
          if(input$gseOther_Select_ema){
            if(is.null(input$gseOther_Table_rows_selected)) return(NULL)
            res <- res_gseOther()$sig_res %>% filter(.,1:nrow(.) %in% input$gseOther_Table_rows_selected)
            # showCategory = nrow(res)
          }else{
          ...
          try(Emapplot(res, showCategory = input$gseOther_ShowCategory_ema, color = input$gseOther_color, layout = "kk"), silent = T)
        })
      })
